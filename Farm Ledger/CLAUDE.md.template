...
A `.claude/hooks/pii-guard.py` hook fires on every `Read` and on `cat`/`head`/`tail`/`less`/`more` Bash commands. When the target file is under the tax data tree, the hook transparently redirects the read to a `<file>.redacted.txt` sidecar generated by `Farm Ledger/pii_redactor.py`.

What this means for you as an agent working in the Wizard's Tower:

- You will see redacted forms by default: SSNs as `[REDACTED]`, DOBs as `**/**/YYYY`, account numbers as `*****4674`, street addresses as `[STREET REDACTED]`. Names, city/state/ZIP, and income amounts remain intact.
- **Do not attempt to circumvent** the guard by copying sensitive content into a new file, asking the user to paste unredacted values, or using a different tool like `Glob`+raw-open. If you need fuller context, tell the user what's missing and let them decide.
- **When writing** to markdown files (Profile.md, Files.md, OpenQuestions.md, etc.), never reintroduce values that came back redacted — leave the field as-is or write the partial form. A field that was redacted to you should remain private to the user in the source file.
- Sidecars regenerate on first read and on every autosync. A stale sidecar is a bug; surface it to the user if you spot it.
